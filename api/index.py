...
4. CRITICAL FORMATTING: The output must have the SAME number of paragraphs as the original template. Use double line breaks (\\n\\n) between EVERY paragraph. If you merge multiple template paragraphs into one, you have FAILED. The greeting is its own line. The observation is its own paragraph. Every other paragraph stays separate.

5. CRITICAL VARIABLES: You may ONLY use variables that already exist in the original template. If the template does not contain {{first_name}}, do NOT add it. If the template does not contain {{company}}, do NOT add it. NEVER invent new variables.
"""
...
def create_sequences():
    ...
    try:
        ...
        def run_in_background(proj_id, contacts_data, templates_data, custom_prompt=None, proj_data=None):
            ...
            # ── FAST PARALLELISM (Respecting Paid Tier QPS Limits) ──
